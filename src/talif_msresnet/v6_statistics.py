"""Prospective paired-seed statistics for the frozen V6 mechanism contract.

The module is isolated from V5.  It implements the V6 replication test, the
...
V6_ALPHA = float(V6_ANALYSIS_CONTRACT["alpha"])
V6_SESOI_PP = float(V6_ANALYSIS_CONTRACT["sesoi_pp"])
V6_CONDITIONS: tuple[str, ...] = V6_ACTIVE_CONDITIONS
V6_PLANNED_SEED_BLOCKS = len(V6_FORMAL_SEEDS)
V6_SIGN_FLIP_ASSIGNMENTS = int(V6_ANALYSIS_CONTRACT["sign_flip"]["assignments"])
V6_SIGN_FLIP_ROLE = str(V6_ANALYSIS_CONTRACT["sign_flip"]["role"])
V6_BOOTSTRAP_RESAMPLES = int(V6_ANALYSIS_CONTRACT["bootstrap"]["resamples"])
V6_BOOTSTRAP_ROLE = str(V6_ANALYSIS_CONTRACT["bootstrap"]["role"])
V6_BOOTSTRAP_RNG = str(V6_ANALYSIS_CONTRACT["bootstrap"]["rng"])
V6_BOOTSTRAP_INTERVAL = str(V6_ANALYSIS_CONTRACT["bootstrap"]["interval"])
V6_BOOTSTRAP_QUANTILE_METHOD = str(
    V6_ANALYSIS_CONTRACT["bootstrap"]["quantile_method"]
)


class V6StatisticsError(ValueError):
    """Raised when input or analysis violates the frozen V6 contract."""
...
class ContrastSpec:
    ...
    def __post_init__(self) -> None:
        ...
        unknown = {self.treatment, self.reference} - set(V6_CONDITIONS)
        if unknown:
            raise ValueError(f"unknown V6 conditions in contrast: {sorted(unknown)}")


V6_REPLICATION_CONTRAST = ContrastSpec("M4-M0", "M4", "M0")
V6_HOLM_CONTRASTS: tuple[ContrastSpec, ...] = (
    ContrastSpec("M4-M2", "M4", "M2"),
    ContrastSpec("M4-M3", "M4", "M3"),
)
V6_SECONDARY_CONTRASTS: tuple[ContrastSpec, ...] = (
...
class BootstrapSensitivity:
    ...
    rng: str = V6_BOOTSTRAP_RNG
    interval: str = V6_BOOTSTRAP_INTERVAL
    quantile_method: str = V6_BOOTSTRAP_QUANTILE_METHOD
    role: str = V6_BOOTSTRAP_ROLE
...
def _validate_frozen_alpha(alpha: float) -> float:
    ...
    if value != V6_ALPHA:
        raise ValueError(f"V6 alpha must remain frozen at {V6_ALPHA:g}")
    return value
...
def _require_contrast_identity(
    ...
    if observed != expected:
        raise V6StatisticsError(
            f"Expected frozen contrast identity {expected}, found {observed}"
        )


def validate_v6_accuracy_blocks(
    frame: pd.DataFrame,
    *,
    expected_conditions: Sequence[str] = V6_CONDITIONS,
    expected_seeds: Sequence[int] = V6_FORMAL_SEEDS,
) -> pd.DataFrame:
    ...
    if missing:
        raise V6StatisticsError(f"Missing V6 columns: {', '.join(missing)}")
    if frame.empty:
        raise V6StatisticsError("No V6 accuracy rows were supplied")
    ...
    if conditions != V6_CONDITIONS:
        raise ValueError(f"expected_conditions must equal the frozen V6 order {V6_CONDITIONS}")
    observed_conditions = set(normalized["condition"])
    if observed_conditions != set(conditions):
        raise V6StatisticsError(
            "V6 conditions differ from the frozen set: "
    ...
    if duplicate.any():
        ...
        raise V6StatisticsError(f"Duplicate V6 seed-condition rows: {examples[:10]}")

    frozen_seeds = tuple(int(value) for value in expected_seeds)
    if frozen_seeds != V6_FORMAL_SEEDS:
        raise ValueError("expected_seeds must equal the exact frozen V6 formal seed order")
    observed_seeds = {int(value) for value in normalized["seed"].unique()}
    if observed_seeds != set(frozen_seeds):
        raise V6StatisticsError(
            "V6 seeds differ from the frozen set: "
...
def paired_contrast(frame: pd.DataFrame, spec: ContrastSpec) -> PairedContrastResult:
    ...
    if n_pairs != V6_PLANNED_SEED_BLOCKS:
        raise V6StatisticsError(
            f"Contrast {spec.name} requires {V6_PLANNED_SEED_BLOCKS} pairs, found {n_pairs}"
        )
...
def paired_tost(
    result: PairedContrastResult,
    *,
    margin_pp: float = V6_SESOI_PP,
    alpha: float = V6_ALPHA,
) -> TOSTResult:
    """Run the frozen paired TOST on an already validated seed contrast."""

    alpha = _validate_frozen_alpha(alpha)
    _require_contrast_identity(result, V6_SECONDARY_CONTRASTS[0])
    if float(margin_pp) != V6_SESOI_PP:
        raise ValueError(f"V6 SESOI must remain frozen at {V6_SESOI_PP:g} pp")
    if margin_pp <= 0.0:
...
def exhaustive_sign_flip(result: PairedContrastResult) -> SignFlipSensitivity:
    ...
    if values.shape != (V6_PLANNED_SEED_BLOCKS,) or not np.isfinite(values).all():
        raise V6StatisticsError(
            f"Sign-flip sensitivity for {result.name} requires exactly "
            f"{V6_PLANNED_SEED_BLOCKS} finite differences"
        )
    signs = np.asarray(
        tuple(product((-1.0, 1.0), repeat=V6_PLANNED_SEED_BLOCKS)), dtype=float
    )
    statistics = (signs * values).mean(axis=1)
    observed = float(values.mean())
    extreme = int(np.count_nonzero(statistics >= observed))
    if len(statistics) != V6_SIGN_FLIP_ASSIGNMENTS:
        raise AssertionError(
            f"The frozen V6 sign-flip space must contain {V6_SIGN_FLIP_ASSIGNMENTS} assignments"
        )
...
def complete_block_bootstrap(result: PairedContrastResult) -> BootstrapSensitivity:
    ...
    if result.name not in expected_by_name or result.name not in V6_BOOTSTRAP_SEEDS:
        raise ValueError(f"Unknown V6 bootstrap contrast: {result.name}")
    _require_contrast_identity(result, expected_by_name[result.name])
    random_seed = int(V6_BOOTSTRAP_SEEDS[result.name])
    rng = np.random.Generator(np.random.PCG64(random_seed))
    indices = rng.integers(
        0,
        V6_PLANNED_SEED_BLOCKS,
        size=(V6_BOOTSTRAP_RESAMPLES, V6_PLANNED_SEED_BLOCKS),
        endpoint=False,
    )
    ...
    return BootstrapSensitivity(
        ...
        n_resamples=V6_BOOTSTRAP_RESAMPLES,
...
def classify_study(
    *,
    ...
    """Apply V6 wording gates; secondary evidence can never rescue the main family."""
    ...
    if route_decision not in {
        ...
        raise V6StatisticsError(f"Unknown V6 route decision: {route_decision}")
    if len(holm_family) != 2 or any(item.superiority_p_holm is None for item in holm_family):
        raise V6StatisticsError("The complete two-member Holm family is required")
    for result, spec in zip(holm_family, V6_HOLM_CONTRASTS):
        ...
        if not np.isfinite(adjusted) or not 0.0 <= adjusted <= 1.0:
            raise V6StatisticsError(f"Invalid Holm-adjusted p-value for {result.name}")
    rejected = tuple(
        item.name
...
def analyze_v6_mechanism_accuracy(
    frame: pd.DataFrame,
    *,
    alpha: float = V6_ALPHA,
    expected_seeds: Sequence[int] = V6_FORMAL_SEEDS,
) -> V6MechanismAnalysis:
    """Run the exact frozen V6 replication, Holm, secondary, and route branches."""

    alpha = _validate_frozen_alpha(alpha)
    normalized = validate_v6_accuracy_blocks(frame, expected_seeds=expected_seeds)
    replication = paired_contrast(normalized, V6_REPLICATION_CONTRAST)
    replication_passed = replication.superiority_p_raw < alpha and replication.estimate_pp > 0.0

    holm_raw = tuple(paired_contrast(normalized, spec) for spec in V6_HOLM_CONTRASTS)
    ...
    secondary = tuple(paired_contrast(normalized, spec) for spec in V6_SECONDARY_CONTRASTS)
    route = classify_route_contribution(secondary[0], alpha=alpha, sesoi_pp=V6_SESOI_PP)
    sensitivities = tuple(
    ...
    return V6MechanismAnalysis(
        alpha=alpha,
        sesoi_pp=V6_SESOI_PP,
...
def mdes_sensitivity(
    *,
    paired_sd_pp: float,
    n_pairs: int = V6_PLANNED_SEED_BLOCKS,
    family_size: int = len(V6_HOLM_CONTRASTS),
    alpha: float = V6_ALPHA,
    power: float = 0.80,
) -> MDESSensitivity:
...
def mdes_grid(
    ...
    """Return the frozen V6 reference and variance-sensitivity MDES values."""
