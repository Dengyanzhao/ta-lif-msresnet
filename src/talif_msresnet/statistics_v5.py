"""Frozen paired-accuracy statistics for the isolated TA-LIF-only v5 study.

V5 keeps five complete C1/C2 pairs per dataset, but the formal seeds are
...
V5_CONDITIONS: tuple[str, str] = ("C1", "C2")
V5_FORMAL_SEEDS_BY_DATASET: Mapping[str, tuple[int, ...]] = {
    "cifar100": (2141022414, 252611743, 1147962214, 931150728, 2106521224),
    "cifar10dvs": (157805125, 2131477013, 1311303835, 417519743, 2086335735),
}
V5_ALL_FORMAL_SEEDS: tuple[int, ...] = tuple(
    seed
    for dataset in ("cifar100", "cifar10dvs")
    for seed in V5_FORMAL_SEEDS_BY_DATASET[dataset]
)
V5_DATASET_GROUPS: Mapping[str, tuple[int, int]] = {
    "cifar100": (20, 6),
    "cifar10dvs": (20, 10),
}
V5_PRIMARY_DATASET = "cifar100"
V5_REPLICATION_DATASET = "cifar10dvs"
V5_ALPHA = 0.05
V5_CONFIDENCE_LEVEL = 0.95
V5_SIGN_FLIP_ASSIGNMENTS = 32
V5_BOOTSTRAP_RESAMPLES = 10_000
V5_BOOTSTRAP_SEEDS: Mapping[str, int] = {
    "cifar100": 894719475,
...
class V5PairedAnalysisError(ValueError):
    """Raised when data violate the frozen v5 paired-analysis contract."""
...
@dataclass(frozen=True)
class V5PairedAnalysis:
    protocol_hash: str
    datasets: Mapping[str, DatasetPairedAnalysis]
    claim_gates: V5ClaimGates
...
def _require_columns(frame: pd.DataFrame, columns: Sequence[str]) -> None:
    ...
    if missing:
        raise V5PairedAnalysisError(f"Missing required v5 columns: {', '.join(missing)}")
...
def _expected_run_id(dataset: str, condition: str, seed: int) -> str:
    depth, time_steps = V5_DATASET_GROUPS[dataset]
    return f"E1_{dataset}_d{depth}_t{time_steps}_{condition}_s{seed}"


def validate_v5_paired_blocks(
    frame: pd.DataFrame,
    *,
    expected_protocol_hash: str,
) -> pd.DataFrame:
    """Validate the exact 20-row v5 matrix without filtering any input row."""
    ...
    if len(frame) != 20:
        raise V5PairedAnalysisError(
            f"The v5 formal analysis requires exactly 20 rows, found {len(frame)}"
        )
    ...
    if empty.any(axis=None):
        raise V5PairedAnalysisError(
            "Missing v5 audit or final-test values at rows "
    ...
    if observed_datasets != set(V5_DATASET_GROUPS):
        failures.append(
    ...
    if observed_conditions != set(V5_CONDITIONS):
        failures.append(
    ...
    if protocol_hashes != {expected_protocol_hash}:
        failures.append(
            "result protocol hashes differ from the isolated v5 protocol: "
    ...
    if duplicate_keys.any():
        ...
        failures.append(f"duplicate v5 result rows: {examples[:10]}")
    ...
    if not statuses.eq("complete").all():
        failures.append("all v5 formal rows must have status='complete'")
    if "failed" in normalized.columns:
        ...
        if failed.isna().any() or not failed.eq(0).all():
            failures.append("all v5 formal rows must record failed=0")

    split_hash_by_dataset: dict[str, str] = {}
    shared_hash_owners: dict[str, tuple[str, int]] = {}
    for dataset, (expected_depth, expected_time_steps) in V5_DATASET_GROUPS.items():
        dataset_rows = normalized.loc[normalized["dataset"] == dataset]
        expected_seeds = V5_FORMAL_SEEDS_BY_DATASET[dataset]
        ...
        observed_seeds = {int(value) for value in dataset_rows["seed"]}
        ...
        for seed in expected_seeds:
            ...
            if len(pair) != 2 or set(pair["condition"]) != set(V5_CONDITIONS):
            ...
            expected_ids = {
                _expected_run_id(dataset, condition, seed) for condition in V5_CONDITIONS
            }
    ...
    if failures:
        raise V5PairedAnalysisError("Invalid v5 paired design: " + "; ".join(failures))
    return normalized
...
def paired_differences_pp(
    ...
    if dataset not in V5_DATASET_GROUPS:
        raise ValueError(f"Unknown v5 dataset: {dataset}")
    selected = normalized.loc[normalized["dataset"] == dataset]
    pivot = selected.pivot(index="seed", columns="condition", values="accuracy_pp")
    pivot = pivot.reindex(V5_FORMAL_SEEDS_BY_DATASET[dataset])
    if tuple(pivot.columns) != V5_CONDITIONS or pivot.isna().any(axis=None):
        raise V5PairedAnalysisError(
...
def complete_pair_bootstrap(
    ...
    *,
    ...
    if dataset not in V5_BOOTSTRAP_SEEDS:
        raise ValueError(f"Unknown v5 bootstrap dataset: {dataset}")
    random_seed = V5_BOOTSTRAP_SEEDS[dataset]
    rng = np.random.Generator(np.random.PCG64(random_seed))
    indices = rng.integers(
        0,
        5,
        size=(V5_BOOTSTRAP_RESAMPLES, 5),
        endpoint=False,
    )
    ...
    return BootstrapSensitivity(
        ...
        n_resamples=V5_BOOTSTRAP_RESAMPLES,
...
def _passes_improvement_gate(result: PairedTResult) -> bool:
    return bool(
        ...
        and result.p_value_one_sided_greater < V5_ALPHA
    )


def analyze_v5_paired_accuracy(
    frame: pd.DataFrame,
    *,
    expected_protocol_hash: str,
) -> V5PairedAnalysis:
    normalized = validate_v5_paired_blocks(
        frame,
    ...
    primary = datasets[V5_PRIMARY_DATASET].paired_t
    replication = datasets[V5_REPLICATION_DATASET].paired_t
