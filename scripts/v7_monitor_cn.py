#!/usr/bin/env python3
"""只读显示 V7 实验进度、单 run 预计完成时间和整体预计完成时间。"""
...
class V7MonitorError(RuntimeError):
    """Raised when the frozen V7 monitoring inputs are malformed."""
...
def _expected_runs(protocol: Mapping[str, Any], stage: str) -> list[dict[str, Any]]:
    raw = generate_v7_pilot_matrix(protocol) if stage == "pilot" else generate_run_matrix(protocol)
    expected_conditions = V7_ACTIVE_CONDITIONS
    result: list[dict[str, Any]] = []
    for item in raw:
        run_id = item.get("run_id")
        condition = item.get("model", {}).get("condition")
        if not isinstance(run_id, str) or condition not in expected_conditions:
            raise V7MonitorError("V7 运行矩阵与冻结条件定义不一致")
        result.append({"run_id": run_id, "condition": str(condition)})
    expected_count = 6 if stage == "pilot" else 48
    if len(result) != expected_count:
        raise V7MonitorError(f"V7 {stage} 矩阵数量错误：{len(result)} != {expected_count}")
    return result
...
def print_snapshot(snapshot: MonitorSnapshot) -> None:
    ...
    pending = sum(run.state == "pending" for run in snapshot.runs)
    print(
        f"V7 中文监控 | 阶段: {snapshot.stage} | 当前时间: {datetime.now().astimezone():%Y-%m-%d %H:%M:%S %z}"
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    protocol = validate_v7_protocol(load_protocol(args.protocol.resolve()))
    while True:
