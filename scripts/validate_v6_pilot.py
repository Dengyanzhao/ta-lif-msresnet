#!/usr/bin/env python3
"""Validate the frozen, non-reportable V6 six-condition mechanism pilot."""
...
PASS_DECISION = "ACCEPT_V6_SIX_CONDITION_120_EPOCH_PILOT_RELEASE_FORMAL_FREEZE"
FAIL_DECISION = "BLOCK_V6_AND_REQUIRE_NEW_PROTOCOL_AND_UNUSED_PILOT_SEED"
INVALID_DECISION = "BLOCK_V6_AND_INVESTIGATE_PILOT_EVIDENCE_INTEGRITY"
...
class PilotValidationError(RuntimeError):
    """Raised when the validator cannot establish the V6 pilot contract."""
...
def _load_generated_matrix(
    *,
    ...
    manifest = _read_json(manifest_path, "V6 pilot matrix manifest")
    raw_rows = manifest.get("runs")
    if not isinstance(raw_rows, list):
        raise PilotValidationError("V6 matrix manifest runs must be a list")
    rows = [row for row in raw_rows if isinstance(row, Mapping)]
    if len(rows) != len(raw_rows):
        failures.append("matrix manifest contains a non-object run")

    expected_raw = generate_v6_pilot_matrix(protocol)
    ...
    if observed_order != expected_order:
        failures.append("matrix manifest is not the ordered six-condition V6 pilot")
    expected_manifest = {
        "protocol": artifact_path_reference(protocol_path, repository_root),
        "protocol_hash": stable_hash(protocol),
        "run_count": len(V6_ACTIVE_CONDITIONS),
...
def _bound_evidence(
    *,
    ...
    if not isinstance(runtime_sources, Mapping):
        ...
        except (OSError, PilotV6Error) as exc:
    ...
    try:
        ...
        health = _read_json(block.health_output, "V6 health report")
        failures.append(f"canonical health validation failed: {exc}")

    plan = _read_json(block.pilot_plan, "V6 pilot plan")
    try:
        ...
        if plan != expected_plan:
            failures.append("pilot plan differs from the exact V6 bound payload")
    return {
        "health": health,
...
def _validate_run(
    *,
    ...
    if planned.as_dict() != expected_config.as_dict():
        integrity.append("generated YAML differs from the in-memory V6 pilot matrix")
...
def validate_pilot(
    *,
    ...
    """Validate the complete V6 pilot and return one fail-closed verdict."""
    ...
    if protocol.get("protocol_version") != 6:
        raise PilotValidationError("This validator accepts only protocol_version 6")
    require_v6_author_freeze(protocol, repository_root=root)
    ...
    if protocol_path != expected_protocol_path:
        raise PilotValidationError(
            f"Protocol path differs from the V6 artifact binding: {protocol_path}"
        )
    if config_dir != expected_config_dir:
        raise PilotValidationError(
            f"Config directory differs from the V6 artifact binding: {config_dir}"
        )
    ...
    if not isinstance(pilot_contract, Mapping):
        raise PilotValidationError("Protocol V6 pilot acceptance mapping is missing")
    if int(pilot_contract.get("epochs", 0)) != 120:
        raise PilotValidationError("The V6 pilot must be exactly 120 epochs")
    ...
    if block.pilot_output_root.is_dir():
        observed_run_ids = {
            ...
            if path.is_dir() and path.name.startswith("E6_")
        }
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if output.exists():
            raise PilotValidationError(
                "V6 pilot validation output already exists; refusing overwrite"
            )
        config_dir = (
        ...
        print(
            f"V6_PILOT_VALIDATION_ERROR: {type(exc).__name__}: {exc}",
            file=sys.stderr,
        )
        return 2
    print(f"V6_PILOT_VALIDATION_{report['status']}")
