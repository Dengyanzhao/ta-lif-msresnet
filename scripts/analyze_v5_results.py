#!/usr/bin/env python3
"""Analyze only the frozen TA-LIF-only v5 C1/C2 formal results."""
...
from analyze_v3_results import (
    _atomic_write_csv,
    audit_v3_final_test_artifacts,
)

from talif_msresnet.aggregate import load_seed_metrics
...
from talif_msresnet.statistics_v5 import (
    V5_ALL_FORMAL_SEEDS,
    V5_ALPHA,
    V5_BOOTSTRAP_RESAMPLES,
    V5_BOOTSTRAP_SEEDS,
    V5_CONDITIONS,
    V5_CONFIDENCE_LEVEL,
    V5_DATASET_GROUPS,
    V5_FORMAL_SEEDS_BY_DATASET,
    V5_PRIMARY_DATASET,
    V5_REPLICATION_DATASET,
    V5_SIGN_FLIP_ASSIGNMENTS,
    analyze_v5_paired_accuracy,
)
from talif_msresnet.utils import atomic_write_json, sha256_file, stable_hash

V5_PROTOCOL_REFERENCE = "configs/protocol_v5_talif_only.yaml"
V5_FORMAL_RESULTS_REFERENCE = "results/formal_v5_talif_only"
V5_ANALYSIS_RESULTS_REFERENCE = "results/analysis/v5_talif_only"
V5_ANALYSIS_JSON = "v5_paired_accuracy_analysis.json"
V5_RAW_DIFFERENCES_CSV = "v5_paired_seed_differences.csv"

_EXPECTED_ARTIFACT_PATHS: Mapping[str, str] = {
    "protocol": V5_PROTOCOL_REFERENCE,
    ...
    "formal_results": V5_FORMAL_RESULTS_REFERENCE,
    "pilot_results": "results/pilot/v5_talif_only",
    "analysis_results": V5_ANALYSIS_RESULTS_REFERENCE,
}


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the isolated v5 paired C2-C1 accuracy analysis."
    )
    parser.add_argument(
        "--protocol",
        type=Path,
        default=REPOSITORY_ROOT / V5_PROTOCOL_REFERENCE,
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=None,
        help="Exact v5 formal-results root; defaults to the protocol-bound path.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=None,
        help="Exact v5 analysis-results root; defaults to the protocol-bound path.",
    )
    return parser
...
def validate_v5_protocol_contract(protocol: Mapping[str, Any]) -> None:
    """Fail closed when executable analysis and the frozen v5 design disagree."""
    ...
    require("protocol_version", protocol.get("protocol_version"), 5)
    require("active_conditions", protocol.get("active_conditions"), list(V5_CONDITIONS))
    require("seeds", protocol.get("seeds"), list(V5_ALL_FORMAL_SEEDS))
    require("output_root", protocol.get("output_root"), V5_FORMAL_RESULTS_REFERENCE)
    ...
    expected_matrix = [
        {
            ...
            "seeds": list(V5_FORMAL_SEEDS_BY_DATASET[dataset]),
        }
        for dataset, (depth, time_steps) in V5_DATASET_GROUPS.items()
    ]
    ...
    require("analysis.alpha", analysis.get("alpha"), V5_ALPHA)
    require(
        "analysis.confidence_level",
        analysis.get("confidence_level"),
        V5_CONFIDENCE_LEVEL,
    )
    require("analysis.primary_dataset", analysis.get("primary_dataset"), V5_PRIMARY_DATASET)
    require(
    ...
    require(
        ...
        sign_flip,
        {
            "assignments": V5_SIGN_FLIP_ASSIGNMENTS,
    ...
    require(
        ...
        bootstrap,
        {
            "resamples": V5_BOOTSTRAP_RESAMPLES,
    ...
    if failures:
        raise ValueError(
            "Frozen v5 protocol and paired-analysis code disagree:\n- "
            + "\n- ".join(failures)
        )
...
def isolated_analysis_paths(
    ...
    *,
    ...
    expected_protocol = (root / V5_PROTOCOL_REFERENCE).resolve()
    if protocol_path.resolve() != expected_protocol:
        raise ValueError(
            f"v5 analysis requires protocol {expected_protocol}; got {protocol_path.resolve()}"
        )
    ...
    if actual_input != expected_input:
        raise ValueError(f"v5 input must be the isolated formal-results root {expected_input}")
    if actual_output != expected_output:
        raise ValueError(f"v5 output must be the isolated analysis-results root {expected_output}")
    if actual_input == actual_output:
        raise ValueError("v5 formal results and analysis output must be isolated directories")
...
def main(argv: list[str] | None = None) -> int:
    ...
    report_path = output_path / V5_ANALYSIS_JSON
    raw_path = output_path / V5_RAW_DIFFERENCES_CSV
    existing = [path for path in (report_path, raw_path) if path.exists()]
    if existing:
        raise SystemExit(
            "Refusing to overwrite existing v5 analysis artifacts:\n- "
    ...
    if not preflight.ok:
        raise SystemExit(
            "v5 analysis requires a frozen final-test protocol:\n- "
            + "\n- ".join(preflight.errors)
        )
    expected_hash = stable_hash(protocol)
    if preflight.protocol_hash != expected_hash:
        raise SystemExit("Preflight protocol hash differs from the loaded v5 protocol")

    metrics = load_seed_metrics(input_path)
    result = analyze_v5_paired_accuracy(
        metrics,
    ...
    for dataset in (V5_PRIMARY_DATASET, V5_REPLICATION_DATASET):
    ...
    payload = {
        "schema": "ta-lif-only-v5-paired-analysis-v1",
        "protocol_version": 5,
        ...
        "n_seed_metric_rows": len(metrics),
