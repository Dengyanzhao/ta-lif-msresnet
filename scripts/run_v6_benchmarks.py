#!/usr/bin/env python3
"""Run the frozen V6 validation-only resource audit for all 48 checkpoints."""
...
from talif_msresnet.benchmark import benchmark_model
...
from talif_msresnet.data import (
...
class V6BenchmarkRunError(RuntimeError):
    """Raised when the resource audit would weaken the frozen V6 boundary."""
...
def _read_json(path: Path, label: str) -> dict[str, Any]:
    try:
        ...
        raise V6BenchmarkRunError(f"Cannot read {label}: {path}: {exc}") from exc
    if not isinstance(value, dict):
        raise V6BenchmarkRunError(f"{label} must be a JSON object: {path}")
    return value
...
def _sha256(value: Any, label: str) -> str:
    if not isinstance(value, str) or len(value) != 64 or any(
        ...
        raise V6BenchmarkRunError(f"{label} must be a SHA-256")
    return value


def _exact_output(path: Path, expected: Path, label: str) -> None:
    if path.resolve() != expected.resolve():
        raise V6BenchmarkRunError(f"{label} is not the canonical V6 path: {path}")


def _hardware_identity(device: torch.device) -> dict[str, str]:
    if device.type != "cuda" or device.index is None:
        raise V6BenchmarkRunError("V6 benchmark requires an explicit CUDA device")
    if not torch.cuda.is_available():
        raise V6BenchmarkRunError("V6 benchmark requires CUDA")
    try:
        ...
        raise V6BenchmarkRunError(f"Cannot identify CUDA benchmark hardware: {exc}") from exc
    fields = [item.strip() for item in output.split(",")]
    if len(fields) != 3 or any(not item for item in fields):
        raise V6BenchmarkRunError("nvidia-smi returned an invalid hardware identity")
    ...
    if device_name not in torch_name and torch_name not in device_name:
        raise V6BenchmarkRunError("nvidia-smi and PyTorch disagree about the selected GPU")
    return {
        "device": str(device),
...
def _finite(value: Any, label: str) -> float:
    if isinstance(value, bool) or not isinstance(value, (int, float)):
        raise V6BenchmarkRunError(f"{label} is not numeric")
    number = float(value)
    if not math.isfinite(number):
        raise V6BenchmarkRunError(f"{label} is not finite")
    return number
...
def _measurement(
    ...
    *,
    ...
    if operations.get("neuron_operation_mode") != neuron_operation_mode:
        raise V6BenchmarkRunError(
            "Benchmark operation accounting mode differs from the frozen V6 condition"
        )
    return {
...
def _load_formal_audit(
    *,
    ...
    expected = generate_run_matrix(protocol)
    expected_ids = expected_run_ids()
    if tuple(str(raw.get("run_id")) for raw in expected) != expected_ids:
        raise V6BenchmarkRunError("V6 generated matrix differs from the frozen run order")
    observed = {
        path.name
    ...
    if observed != set(expected_ids):
        raise V6BenchmarkRunError(
            "Formal result directory set differs from the V6 matrix: "
    ...
    for raw in expected:
        ...
        for required in ("best.pt", "run_manifest.json", "seed_metrics.json"):
            if not (run_dir / required).is_file():
                raise V6BenchmarkRunError(f"{run_id}: missing formal artifact {required}")
        if (run_dir / "final_test.json").exists() or (run_dir / "final_test.in_progress.json").exists():
            raise V6BenchmarkRunError(
                f"{run_id}: benchmark must complete before any V6 test transaction"
            )
        ...
        if metrics.get("status") != "complete" or manifest.get("status") != "complete":
            raise V6BenchmarkRunError(f"{run_id}: formal training is not complete")
        if metrics.get("test_accuracy") not in (None, ""):
            raise V6BenchmarkRunError(f"{run_id}: test metric exists before benchmark completion")
        checkpoint_path = run_dir / "best.pt"
        checkpoint = load_checkpoint(checkpoint_path, map_location="cpu")
        raw_checkpoint_config = checkpoint.get("config")
        if not isinstance(raw_checkpoint_config, Mapping):
            raise V6BenchmarkRunError(f"{run_id}: checkpoint configuration is missing")
        ...
        if checkpoint_config.runtime.run_id != run_id or checkpoint_config.config_hash != resolved.config_hash:
            raise V6BenchmarkRunError(f"{run_id}: checkpoint differs from the frozen matrix")
        config_hash = _sha256(str(metrics.get("config_hash", "")), f"{run_id} config hash")
        if config_hash != resolved.config_hash or str(manifest.get("config_hash", "")) != config_hash:
            raise V6BenchmarkRunError(f"{run_id}: config hash differs across formal artifacts")
        if checkpoint.get("config_hash") != config_hash:
            raise V6BenchmarkRunError(f"{run_id}: checkpoint config hash is invalid")
        environment = _sha256(
            str(metrics.get("training_environment_sha256", "")),
            f"{run_id} training environment",
        )
        split = _sha256(str(metrics.get("split_manifest_sha256", "")), f"{run_id} split manifest")
        shared = _sha256(str(metrics.get("shared_weight_sha256", "")), f"{run_id} shared init")
        ...
        if not isinstance(manifest_environment, Mapping) or manifest_environment.get(
            ...
            raise V6BenchmarkRunError(f"{run_id}: manifest training environment differs")
        if manifest.get("split_manifest_sha256") != split or manifest.get("shared_weight_sha256") != shared:
            raise V6BenchmarkRunError(f"{run_id}: manifest split or initialization differs")
    ...
    if len(environments) != 1 or len(splits) != 1:
        raise V6BenchmarkRunError("V6 formal matrix has mixed training environments or data splits")
    if any(len(values) != 1 for values in shared_by_seed.values()):
        raise V6BenchmarkRunError("V6 conditions do not share initialization within each seed")
    return rows
...
def _load_fixed_batch(
    *,
    ...
    if not path.is_file():
        raise V6BenchmarkRunError(
            "Fixed V6 validation batch is missing; run scripts/export_v6_validation_batch.py"
        )
    ...
    if metadata.get("input_seed") != V6_BENCHMARK_CONTRACT["input_seed"] or int(
        metadata.get("batch_size", 0)
    ) != 128:
        raise V6BenchmarkRunError("Fixed V6 validation batch has the wrong seed or size")
    for row in audit_rows:
        if row["split_manifest_sha256"] != split:
            raise V6BenchmarkRunError("V6 benchmark cannot use a mixed split matrix")
...
def _result_payload(
    *,
    ...
    return {
        "schema": "ta-lif-msresnet-v6-validation-benchmark-v1",
        "artifact_class": "V6_VALIDATION_ONLY_CHECKPOINT_RESOURCE_AUDIT",
        "status": "complete",
        "protocol_version": 6,
        ...
        "input": {
            ...
            "input_seed": V6_BENCHMARK_CONTRACT["input_seed"],
            "batch_size": 128,
        },
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.device != "cuda:0":
            raise V6BenchmarkRunError("V6 benchmark device is frozen as cuda:0")
        protocol_path = args.protocol.resolve()
        expected_protocol = (PROJECT_ROOT / V6_ARTIFACT_PATHS["protocol"]).resolve()
        _exact_output(protocol_path, expected_protocol, "protocol")
        ...
        if not preflight.ok:
            raise V6BenchmarkRunError("V6 benchmark preflight failed: " + "; ".join(preflight.errors))
        verify_formal_freeze(
            project_root=PROJECT_ROOT,
            protocol_path=protocol_path,
            matrix_dir=PROJECT_ROOT / V6_ARTIFACT_PATHS["formal_matrix"],
        )
        manifest_path = (PROJECT_ROOT / V6_ARTIFACT_PATHS["freeze_manifest"]).resolve()
        if not manifest_path.is_file():
            raise V6BenchmarkRunError("V6 freeze manifest is missing")
        results_root = formal_results_root(protocol, PROJECT_ROOT)
        if not results_root.is_dir():
            raise V6BenchmarkRunError("V6 formal results root is missing")
        audit_rows = _load_formal_audit(
            protocol=protocol,
        ...
        if receipt_path.exists():
            ...
            print(f"V6_BENCHMARK_RECEIPT_ALREADY_PASS={receipt_path}")
            return 0
        ...
        for index, row in enumerate(audit_rows, start=1):
            ...
            result = benchmark_model(
                model,
                input_factory,
                batch_sizes=tuple(V6_BENCHMARK_CONTRACT["batch_sizes"]),
                warmup_iterations=int(V6_BENCHMARK_CONTRACT["warmup_iterations"]),
                iterations=int(V6_BENCHMARK_CONTRACT["timed_iterations"]),
                device=device,
                neuron_operation_mode=V6_OPERATION_MODE_BY_CONDITION[config.model.condition],
                energy_constants=None,
            )
            operation_mode = V6_OPERATION_MODE_BY_CONDITION[config.model.condition]
        ...
        print(f"V6_BENCHMARK_RECEIPT_PASS={receipt_path}")
        return 0
    except (V6BenchmarkError, V6BenchmarkRunError, OSError, ValueError, RuntimeError) as exc:
        print(f"V6_BENCHMARK_BLOCKED: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
