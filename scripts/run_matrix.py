...
def run_matrix(
    ...
    if protocol_version in (3, 4) and dry_run:
        raise ValueError(
    ...
    if pilot_active:
        conditions_label = "C1/C2" if v3_pilot or v4_pilot else "C1-C4"
        print(
    ...
    if config_path is None:
        ...
        elif protocol_version in (3, 4) and not dry_run and not pilot_active:
            _validate_v2_generated_matrix(
                protocol=protocol,
                protocol_path=protocol_path,
                config_dir=config_dir,
    ...
    for index, row in enumerate(rows, start=1):
        ...
        try:
            action, resume_checkpoint = _continuation_action(
                ...
                forbid_fresh_after_attempt=protocol_version in (3, 4) and not dry_run,
            )
