#!/usr/bin/env python3
"""Create the offline source-and-data package for a blank V6 RTX 5090 instance.

The package intentionally contains only the committed V6 source history and
...
PACKAGE_ROOT_NAME = "ta-lif-msresnet-v6-instance-input"
PACKAGE_SCHEMA = "ta-lif-msresnet-v6-instance-package-v1"


class V6InstancePackageError(RuntimeError):
    """Raised when an offline V6 package cannot be safely created."""
...
def _git(*args: str, cwd: Path = PROJECT_ROOT) -> str:
    ...
    if completed.returncode:
        ...
        raise V6InstancePackageError(f"git {' '.join(args)} failed: {detail}")
...
def _inside(root: Path, path: Path, label: str) -> Path:
    ...
    try:
        ...
        raise V6InstancePackageError(f"{label} escapes the repository: {resolved}") from exc
    return resolved
...
def _required_data_paths(project_root: Path) -> tuple[Path, ...]:
    ...
    paths = tuple(
        _inside(project_root, project_root / item, "V6 package input") for item in relative_paths
    )
    missing = [str(path) for path in paths if not path.is_file()]
    if missing:
        raise V6InstancePackageError(
...
def _verify_provenance(project_root: Path, paths: Iterable[Path]) -> None:
    ...
    try:
        ...
        raise V6InstancePackageError(f"Cannot read CIFAR-100 provenance: {exc}") from exc
    if not isinstance(provenance, dict):
        raise V6InstancePackageError("CIFAR-100 provenance must be a JSON object")
    ...
    if (
        ...
        raise V6InstancePackageError("CIFAR-100 provenance is incomplete")
    ...
    for relative, expected_hash in expected.items():
        ...
        if len(expected_hash) != 64:
            raise V6InstancePackageError(f"CIFAR-100 provenance lacks a valid hash for {relative}")
...
def _assert_fresh_v6_artifact_state(project_root: Path, protocol: dict[str, Any]) -> None:
    ...
    if present:
        raise V6InstancePackageError(
            "Refusing to package a non-pristine V6 artifact state:\n- " + "\n- ".join(present)
        )
...
def _write_readme(path: Path) -> None:
    path.write_text(
        "This directory is the payload of a V6 blank-instance package.\n\n"
        "1. Verify SHA256SUMS.txt before extracting or using its contents.\n"
        "2. Clone source/ta-lif-msresnet-v6.bundle into /hy-tmp/ta-lif-msresnet.\n"
        "3. Copy data/ and environment/CIFAR100_SOURCE_PROVENANCE.json into that clone.\n"
        "4. Follow V6_MECHANISM_5090_RUNBOOK.md from the committed source.\n\n"
        "The package excludes all V6 experiment outputs by design.\n",
        encoding="utf-8",
        newline="\n",
    )
...
def create_package(*, project_root: Path, protocol_path: Path, output: Path) -> dict[str, Any]:
    ...
    protocol_path = _inside(project_root, protocol_path, "V6 protocol")
    output = output.expanduser().resolve()
    if output.exists():
        raise V6InstancePackageError(f"Refusing to overwrite existing package: {output}")
    sidecar = Path(f"{output}.sha256")
    if sidecar.exists():
        raise V6InstancePackageError(f"Refusing to overwrite existing checksum sidecar: {sidecar}")
    try:
        output.relative_to(project_root)
    except ValueError:
        pass
    else:
        raise V6InstancePackageError("Package output must be outside the repository")
    if output.suffixes[-2:] != [".tar", ".gz"]:
        raise V6InstancePackageError("V6 instance package output must end in .tar.gz")
    if not output.parent.is_dir():
        raise V6InstancePackageError(f"Package parent directory is missing: {output.parent}")
    ...
    if protocol_path != expected_protocol:
        raise V6InstancePackageError(f"V6 protocol path must be {expected_protocol}")
    if _git("status", "--porcelain=v1", cwd=project_root):
        raise V6InstancePackageError(
            "V6 source package requires a clean tracked and untracked worktree"
        )
    branch = _git("branch", "--show-current", cwd=project_root)
    if not branch:
        raise V6InstancePackageError("V6 source package requires a named checked-out branch")
    commit = _git("rev-parse", "HEAD", cwd=project_root)
    _assert_fresh_v6_artifact_state(project_root, protocol)
    data_paths = _required_data_paths(project_root)
    _verify_provenance(project_root, data_paths)

    temporary_parent = Path(tempfile.mkdtemp(prefix=".v6-instance-package-", dir=output.parent))
    try:
        ...
        bundle = source_dir / "ta-lif-msresnet-v6.bundle"
        ...
        manifest = {
            ...
            "source": {
                "branch": branch,
                "commit": commit,
                "bundle": "source/ta-lif-msresnet-v6.bundle",
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (OSError, ValueError, V6InstancePackageError) as exc:
        print(f"V6_INSTANCE_PACKAGE_BLOCKED: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 2
    print(f"V6_INSTANCE_PACKAGE_CREATED={result['output']}")
    print(f"V6_INSTANCE_PACKAGE_SHA256={result['archive_sha256']}")
    print(f"V6_INSTANCE_PACKAGE_SIDECAR={result['checksum_sidecar']}")
    print(f"V6_INSTANCE_PACKAGE_SOURCE={result['branch']}@{result['commit']}")
    return 0
