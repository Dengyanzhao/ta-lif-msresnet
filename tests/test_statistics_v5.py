...
from talif_msresnet.statistics_v5 import (
    ...
    V5PairedAnalysisError,
    analyze_v5_paired_accuracy,
    complete_pair_bootstrap,
    validate_v5_paired_blocks,
)

PROTOCOL_HASH = "v5-protocol-hash"
...
def v5_metrics(
    *,
    ...
    for dataset, depth, time_steps, base_accuracy, differences in specifications:
        seeds = V5_FORMAL_SEEDS_BY_DATASET[dataset]
...
def test_v5_analysis_preserves_dataset_specific_seed_order() -> None:
    ...
    assert tuple(item.seed for item in primary.raw_differences) == (
        V5_FORMAL_SEEDS_BY_DATASET["cifar100"]
    )
    assert tuple(item.seed for item in replication.raw_differences) == (
        V5_FORMAL_SEEDS_BY_DATASET["cifar10dvs"]
    )
...
def test_v5_bootstrap_is_deterministic_and_dataset_bound() -> None:
    ...
    assert first == second
    assert first.random_seed == V5_BOOTSTRAP_SEEDS["cifar100"]
    assert dvs.random_seed == V5_BOOTSTRAP_SEEDS["cifar10dvs"]
    assert first.n_resamples == 10_000


def test_v5_validation_rejects_cross_dataset_seed_substitution() -> None:
    frame = v5_metrics()
    cifar_seed = V5_FORMAL_SEEDS_BY_DATASET["cifar100"][0]
    dvs_seed = V5_FORMAL_SEEDS_BY_DATASET["cifar10dvs"][0]
    frame.loc[
        (frame["dataset"] == "cifar100") & (frame["seed"] == cifar_seed),
        "seed",
    ] = dvs_seed

    with pytest.raises(V5PairedAnalysisError, match="formal seeds differ"):
        validate_v5_paired_blocks(frame, expected_protocol_hash=PROTOCOL_HASH)


def test_v5_validation_rejects_partial_pair_and_c3() -> None:
    frame = v5_metrics().iloc[:-1].copy()
    with pytest.raises(V5PairedAnalysisError, match="exactly 20 rows"):
        validate_v5_paired_blocks(frame, expected_protocol_hash=PROTOCOL_HASH)

    frame = v5_metrics()
    frame.loc[0, "condition"] = "C3"
    with pytest.raises(V5PairedAnalysisError, match="conditions must be exactly C1/C2"):
        validate_v5_paired_blocks(frame, expected_protocol_hash=PROTOCOL_HASH)


def test_v5_replication_cannot_rescue_failed_primary() -> None:
    result = analyze_v5_paired_accuracy(
        v5_metrics(
