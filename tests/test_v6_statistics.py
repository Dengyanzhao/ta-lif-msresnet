...
from talif_msresnet.v6_statistics import (
    ...
    mdes_sensitivity,
    paired_contrast,
    validate_v6_accuracy_blocks,
)

ROOT = Path(__file__).resolve().parents[1]
SEEDS = V6_FORMAL_SEEDS
...
def test_frozen_contract_has_six_conditions_eight_blocks_and_direct_contrasts() -> None:
    assert V6_CONDITIONS == ("M0", "M1", "M2", "M3", "M4", "PLIF")
    assert V6_PLANNED_SEED_BLOCKS == 8
    assert (
        V6_REPLICATION_CONTRAST.name,
        V6_REPLICATION_CONTRAST.treatment,
        V6_REPLICATION_CONTRAST.reference,
    ) == ("M4-M0", "M4", "M0")
    assert tuple(
        (spec.name, spec.treatment, spec.reference) for spec in V6_HOLM_CONTRASTS
    ) == (
        ("M4-M2", "M4", "M2"),
        ("M4-M3", "M4", "M3"),
    )
    assert tuple(spec.name for spec in V6_SECONDARY_CONTRASTS) == (
        "M1-M0",
        "PLIF-M0",
        "M4-PLIF",
    )
...
def test_protocol_config_and_statistics_share_one_frozen_analysis_contract() -> None:
    protocol = yaml.safe_load(
        (ROOT / "configs" / "protocol_v6_mechanism.yaml").read_text(encoding="utf-8")
    )
    validated = validate_v6_protocol(protocol)

    assert validated["analysis"] == V6_ANALYSIS_CONTRACT
    assert tuple(validated["seeds"]) == V6_FORMAL_SEEDS
    ...
    assert validated["analysis"]["bootstrap"]["seeds"] == V6_BOOTSTRAP_SEEDS
    assert validated["analysis"]["power_planning"]["paired_seed_count"] == 8


def test_v6_requires_exact_complete_seed_blocks() -> None:
    frame = mechanism_frame()
    normalized = validate_v6_accuracy_blocks(frame, expected_seeds=SEEDS)
    assert len(normalized) == len(SEEDS) * len(V6_CONDITIONS) == 48

    incomplete = frame.loc[
        ~((frame["seed"] == SEEDS[0]) & (frame["condition"] == "M4"))
    ]
    with pytest.raises(V6StatisticsError, match="Incomplete V6 seed blocks"):
        validate_v6_accuracy_blocks(incomplete, expected_seeds=SEEDS)


def test_v6_rejects_duplicate_wrong_seed_and_invalid_accuracy() -> None:
    frame = mechanism_frame()
    duplicate = pd.concat([frame, frame.iloc[[0]]], ignore_index=True)
    with pytest.raises(V6StatisticsError, match="Duplicate V6 seed-condition rows"):
        validate_v6_accuracy_blocks(duplicate, expected_seeds=SEEDS)

    wrong_seed = frame.copy()
    wrong_seed.loc[wrong_seed["seed"] == SEEDS[-1], "seed"] = 9999
    with pytest.raises(V6StatisticsError, match="V6 seeds differ from the frozen set"):
        validate_v6_accuracy_blocks(wrong_seed, expected_seeds=SEEDS)

    invalid_accuracy = frame.copy()
    invalid_accuracy.loc[0, "test_accuracy_pp"] = np.nan
    with pytest.raises(V6StatisticsError, match="finite percentage points"):
        validate_v6_accuracy_blocks(invalid_accuracy, expected_seeds=SEEDS)

    with pytest.raises(ValueError, match="exact frozen V6 formal seed order"):
        validate_v6_accuracy_blocks(frame, expected_seeds=tuple(range(8)))


def test_supported_analysis_uses_only_the_two_member_holm_family() -> None:
    analysis = analyze_v6_mechanism_accuracy(mechanism_frame(), expected_seeds=SEEDS)
...
def test_m4_minus_m3_is_the_bank_matched_history_index_contrast() -> None:
    frame = validate_v6_accuracy_blocks(mechanism_frame(), expected_seeds=SEEDS)
    result = paired_contrast(frame, V6_HOLM_CONTRASTS[1])

    assert result.name == "M4-M3"
    assert result.treatment == "M4"
    assert result.reference == "M3"
    assert result.estimate_pp > V6_SESOI_PP
...
def test_route_branch_can_establish_practical_equivalence() -> None:
    ...
    frame = validate_v6_accuracy_blocks(
        mechanism_frame(route_differences=route_differences), expected_seeds=SEEDS
    )
    route_result = paired_contrast(frame, V6_SECONDARY_CONTRASTS[0])
    conclusion = classify_route_contribution(route_result)

    assert route_result.estimate_pp < V6_SESOI_PP
    assert conclusion.decision == "route_practical_equivalence"
    assert conclusion.tost.equivalent
    assert conclusion.tost.ci90_low_pp > -V6_SESOI_PP
    assert conclusion.tost.ci90_high_pp < V6_SESOI_PP
...
def test_failed_replication_blocks_mechanism_wording() -> None:
    analysis = analyze_v6_mechanism_accuracy(
...
def test_study_gate_requires_complete_adjusted_holm_family() -> None:
    ...
    with pytest.raises(V6StatisticsError, match="complete two-member Holm family"):
        classify_study(
    ...
    with pytest.raises(V6StatisticsError, match="frozen contrast identity"):
        classify_study(
            replication_passed=True,
    ...
    with pytest.raises(V6StatisticsError, match="Unknown V6 route decision"):
        classify_study(
            replication_passed=True,
...
def test_zero_variance_fails_closed_instead_of_inventing_infinite_t() -> None:
    ...
    with pytest.raises(V6StatisticsError, match="zero paired variance"):
        paired_contrast(normalized, V6_HOLM_CONTRASTS[1])


def test_sign_flip_and_bootstrap_are_frozen_reproducible_sensitivities() -> None:
    frame = validate_v6_accuracy_blocks(mechanism_frame(), expected_seeds=SEEDS)
    result = paired_contrast(frame, V6_REPLICATION_CONTRAST)
    sign_flip = exhaustive_sign_flip(result)
    first = complete_block_bootstrap(result)
    second = complete_block_bootstrap(result)

    assert sign_flip.assignments == V6_SIGN_FLIP_ASSIGNMENTS == 256
    ...
    assert first == second
    assert first.n_resamples == V6_BOOTSTRAP_RESAMPLES == 10_000
    assert first.n_complete_blocks == 8
    assert first.ci95_low_pp <= first.observed_mean_pp <= first.ci95_high_pp
    assert first.role == V6_ANALYSIS_CONTRACT["bootstrap"]["role"]
    assert first.rng == V6_ANALYSIS_CONTRACT["bootstrap"]["rng"]
    assert first.interval == V6_ANALYSIS_CONTRACT["bootstrap"]["interval"]
    assert first.quantile_method == V6_ANALYSIS_CONTRACT["bootstrap"]["quantile_method"]


def test_analysis_rejects_post_freeze_alpha_change() -> None:
    with pytest.raises(ValueError, match="alpha must remain frozen"):
        analyze_v6_mechanism_accuracy(mechanism_frame(), alpha=0.10)
