...
import validate_v6_source_release as source_release
...
def _write_package(path: Path) -> None:
    ...
    contents = {
        "README.txt": b"readme\n",
        "source/ta-lif-msresnet-v6.bundle": b"bundle\n",
        ...
        json.dumps(
            {
                ...
                "source": {
                    "branch": "v6-mechanism-study",
                    "commit": "a" * 40,
                    "bundle": "source/ta-lif-msresnet-v6.bundle",
                    "bundle_sha256": _sha256(contents["source/ta-lif-msresnet-v6.bundle"]),
                },
                "protocol": {
...
def test_offline_package_validation_checks_embedded_hashes(tmp_path: Path) -> None:
    package = tmp_path / "v6-input.tar.gz"
...
def test_offline_package_validation_rejects_bad_sidecar(tmp_path: Path) -> None:
    ...
    with pytest.raises(source_release.V6SourceReleaseError, match="checksum sidecar"):
...
def test_offline_package_validation_binds_current_source_commit(tmp_path: Path) -> None:
    ...
    with pytest.raises(source_release.V6SourceReleaseError, match="source commit"):
