...
def v6_runs():
    protocol = load_protocol("configs/protocol_v6_mechanism.yaml")
...
def test_v6_condition_order_and_neuron_implementation_are_exact(v6_runs) -> None:
    assert tuple(v6_runs) == V6_ACTIVE_CONDITIONS
    for condition, config in v6_runs.items():
...
def test_v6_all_conditions_share_conv_bn_classifier_initialization(v6_runs) -> None:
    hashes = {
...
def test_v6_optimizer_groups_only_real_adaptive_parameters(v6_runs) -> None:
    expected_fragments = {
        "M2": {".center", ".raw_width"},
        "M3": {".center", ".raw_width"},
        "M4": {".center", ".raw_width"},
...
def test_v6_artifact_output_path_accepts_only_bound_absolute_equivalent(tmp_path: Path) -> None:
    protocol = load_protocol("configs/protocol_v6_mechanism.yaml")
...
def test_v6_artifact_output_path_rejects_a_different_absolute_directory(tmp_path: Path) -> None:
    protocol = load_protocol("configs/protocol_v6_mechanism.yaml")
    raw = generate_run_matrix(protocol)[0]
    artifact = dict(raw)
    artifact["runtime"] = {
        **raw["runtime"],
        "output_dir": str((tmp_path / "results" / "wrong-v6-root").resolve()),
    }
