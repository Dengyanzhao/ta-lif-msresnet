# V7 statistical analysis audit

## Contract status

The executable statistical implementation follows
`configs/protocol_v7_mechanism.yaml` and `src/talif_msresnet/config_v7.py`.
It is isolated from V5: V5 artifacts, seeds, protocol fields, and conclusions
are neither modified nor pooled into V7.
...
the validated V7 configuration contract rather than maintained as a second
editable copy.

The V7 primary unit is one complete six-condition seed block. The frozen core
...
new protocol version; they cannot be repaired inside the frozen V7 study.
...
`src/talif_msresnet/v7_statistics.py` implements the exact block validator,
